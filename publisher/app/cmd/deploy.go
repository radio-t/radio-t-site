...
func (d *Deploy) Do(episodeNum int) error {
	...
	cmdStr := fmt.Sprintf(`git pull && git add . && git diff --staged --exit-code --quiet || git commit -m "episode %d" && git push`,
		episodeNum)
	d.Run(cmdStr)
	...
	exportLogCmd := fmt.Sprintf(`"docker exec -i super-bot /srv/telegram-rt-bot %s --dbg --export-num=%d --export-path=/srv/html"`,
		strings.Join(slParams, " "), episodeNum)
	d.Run("ssh umputun@master.radio-t.com", exportLogCmd)
...
// archiveNews invokes news-api like DELETE https://news.radio-t.com/api/v1/news/active/last/12
